#
# Purpose: Load ECAPA embeddings and learn their emotion classes (provided their labels)
...
#
# Usage: python ecapa_ser_experiment.py [test1/test2] <msp/vox> <dataset root>
# Example: python ecapa_ser_experiment.py test2 vox /research/iprobe/datastore/datasets/speech/utd-msppodcast_v1.8/
# 
...
with open('../../ExperimentData/pretrained_ecapa_'+variation+'/val_speaker_embeddings.pickle', 'rb') as handle:
    embeds = pickle.load(handle)

with open('../../ExperimentData/pretrained_ecapa_'+variation+'/'+test_set+'_speaker_embeddings.pickle', 'rb') as handle:
    test_embeds = pickle.load(handle)

print(len(embeds), 'train embeddings loaded')
print(len(test_embeds), 'test embeddings loaded')

dataset_root = sys.argv[3] + '/Labels/labels_concensus.csv'
if not dataset_root:
    pass
else:
    ...
    if test_set == 'test1':
        ...
        plt.savefig('./'+test_set+'_'+variation+'_ser_confusion_matrix.png')
